...
function getNodemailerTransport() {
    ...
    if (!nodemailerTransport) {
        nodemailerTransport = nodemailer_1.default.createTransport({
 ...
 */
function buildReminderHtml({ recipientName, noteTitle, noteContent, checklist, reminderTime, appUrl, }) {
    ...
    const checklistHtml = checklist && checklist.length > 0
        ...
        <div style="margin-top: 18px; padding-top: 14px; border-top: 1px solid #e2e8f0;">
          <strong style="font-size: 13px; color: #023859; text-transform: uppercase; letter-spacing: 0.5px;">Checklist (${checklist.filter((c) => c.completed).length}/${checklist.length} completed):</strong>
          <ul style="margin: 8px 0 0 0; padding-left: 20px; color: #334155; font-size: 14px; line-height: 1.6;">
            ${checklist
            .map((item) => `<li style="${item.completed ? 'text-decoration: line-through; color: #94a3b8;' : ''}">${item.text}</li>`)
            .join('')}
          </ul>
        </div>
      `
...
async function sendReminderEmail(options) {
    ...
    console.warn(`[EmailService] ⚠️ Cannot send reminder to ${to}: Neither RESEND_API_KEY nor (SMTP_USER & SMTP_PASS) is configured in environment variables.`);
    return {
        success: false,
